...
let deleteReceiptPending = false;
...
function setupFileInputListeners() {
    ...
    document.getElementById('editReceipt').addEventListener('change', function() {
        ...
        if (this.files && this.files[0]) {
            ...
            label.textContent = this.files[0].name;
            label.classList.add('has-file');
            // Cancel pending deletion if the user picks a replacement file
            if (deleteReceiptPending) {
                deleteReceiptPending = false;
                const expense = allExpenses.find(exp => exp.id === editingId);
                document.getElementById('currentReceiptHint').textContent = expense && expense.receipt_path
...
function openEditModal(id) {
    ...
    // Show the current receipt filename as a hint, and toggle the delete button
    const hint = document.getElementById('currentReceiptHint');
    const deleteBtn = document.getElementById('deletePictureBtn');
    if (expense.receipt_path) {
        hint.textContent = `Current: ${expense.receipt_path.split('/').pop()}`;
        deleteBtn.style.display = 'inline-block';
    } else {
        hint.textContent = 'No picture attached';
...
// Marks the current receipt for deletion on next save
// Hides the delete button and updates the hint so the user gets clear feedback
function markReceiptForDeletion() {
    deleteReceiptPending = true;
    document.getElementById('currentReceiptHint').textContent = 'Picture will be removed on save';
    document.getElementById('deletePictureBtn').style.display = 'none';
    document.getElementById('editReceipt').value = '';
    document.getElementById('editFileLabel').textContent = 'Choose a file...';
    document.getElementById('editFileLabel').classList.remove('has-file');
...
async function saveEdit() {
    ...
    if (deleteReceiptPending && !receiptFile) {
